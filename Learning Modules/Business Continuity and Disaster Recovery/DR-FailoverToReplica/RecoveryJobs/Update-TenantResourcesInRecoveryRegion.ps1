...
foreach($server in $originRegionServers)
{
  ...
  $recoveryServerExists = Get-AzureRmResource -ResourceGroupNameEquals $WingtipRecoveryResourceGroup -ResourceNameEquals $recoveryServerName -ResourceType "Microsoft.Sql/servers"
...
foreach($pool in $originRegionElasticPools)
{
  ...
  $recoveryPoolExists = Get-AzureRmResource -ResourceGroupNameEquals $WingtipRecoveryResourceGroup -ResourceNameEquals "$recoveryServerName/$($pool.ElasticPoolName)"
