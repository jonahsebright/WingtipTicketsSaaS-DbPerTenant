...
foreach($server in $recoveryRegionServers)
{
  ...
  $originServerExists = Get-AzureRmResource -ResourceGroupNameEquals $wtpUser.ResourceGroupName -ResourceNameEquals $originServerName -ResourceType "Microsoft.Sql/servers"
...
foreach($pool in $recoveryRegionElasticPools)
{
  ...
  $originPoolExists = Get-AzureRmResource -ResourceGroupNameEquals $wtpUser.ResourceGroupName -ResourceNameEquals "$originServerName/$($pool.ElasticPoolName)"
