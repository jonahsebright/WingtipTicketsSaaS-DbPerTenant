...
$existingGoldenTenantDatabase = Get-AzureRmResource -ResourceGroupNameEquals $ResourceGroupName -ResourceType "Microsoft.sql/servers/databases" -ResourceNameContains $config.GoldenTenantDatabaseName
$existingNewTenantPool = Get-AzureRmResource -ResourceGroupNameEquals $ResourceGroupName -ResourceType "Microsoft.sql/servers/elasticpools" -ResourceNameEquals "$ServerName/Pool1"
